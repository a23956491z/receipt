...
if [[ ${confirm} == 'G' ]] ; then
  ...
  cat source.html | sed 's/spanid/span id/g'  |  tidy -imq --show-errors 0 | grep 'class=".*">[0-9]*<' | \
    sed 's/class=\".*\"//g;s/<h.*>//g;s/[^0-9]//g;5,$ s/\w/:/g;s/\ /:/g;/:/d;/^$/d;s/\ /:/;2 s/^/特別獎：/;3 s/^/特獎：/;4 s/^/增開陸獎：/' \
    > fetch_exp_head
  cat source.html | sed 's/spanid/span id/g'  |  tidy -imq --show-errors 0 | sed 's/、/ /g' | grep '".*">[0-9].*<' | \
